...
case class Parent_Tile(
                        name: String,
                        neighbourNames: Set[String] = Set.empty,
                        connections: List[direction] = List(),
                      ) {

  def addNeighbourName(neighbourName: String): Parent_Tile =
    copy(neighbourNames = neighbourNames + neighbourName)

  def add_connection(dir: direction): Parent_Tile =
    copy(connections = connections :+ dir)
}
